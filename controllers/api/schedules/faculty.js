...
exports.getAllScheduleByCourse = async (req, res, next) => {
  try {
    const faculty = await Schedule.aggregate([
      {
        $match: {
          "semesters._id": mongoose.Types.ObjectId(req.params.semester),
        },
      ...
      {
        $group: {
          _id: { faculty: "$faculty", course: "$course" },
          data: { $push: "$$ROOT" },
        },
      },
      {
    ...
    const facultyReturn = faculty.map((element) => {
      return {
        _id: element._id,
    ...
    res.status(200).json({ faculty: facultyReturn });
  } catch (error) {
    console.log(error);
    res.status(500).json({ msg: "Something went wrong" });
  }
